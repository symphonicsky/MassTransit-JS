...
import {
  AsyncReceiveEndpoint,
  AsyncReceiveEndpointConfigurator,
} from "./AsyncReceiveEndpoint";
...
export interface Bus {
  ...
  receiveEndpoint(
    queueName: string,
    config: (endpoint: ReceiveEndpointConfigurator) => void,
    options?: ReceiveEndpointOptions
  ): void;
  asyncReceiveEndpoint(
    queueName: string,
    config: (endpoint: AsyncReceiveEndpointConfigurator) => void,
    options?: ReceiveEndpointOptions
  ): void;

  sendEndpoint(args: SendEndpointArguments): SendEndpoint;

  requestClient<TRequest extends MessageMap, TResponse extends MessageMap>(
    args: RequestClientArguments
...
class MassTransitBus extends EventEmitter implements Bus {
  ...
  receiveEndpoint(
    ...
    let endpoint = new ReceiveEndpoint(this, queueName, cb, {
      ...defaultReceiveEndpointOptions,
      ...options,
    });
  ...
  asyncReceiveEndpoint(
    ...
    let endpoint = new AsyncReceiveEndpoint(this, queueName, cb, {
      ...defaultReceiveEndpointOptions,
      ...options,
    });
  ...
  requestClient<TRequest extends MessageMap, TResponse extends MessageMap>(
    ...
    return new RequestClient<TRequest, TResponse>(
      this.busEndpoint,
      sendEndpoint,
      args.requestType,
      args.responseType
  ...
  async restart(): Promise<void> {
    ...
    this.connect().catch(
      () => (err: { message: any; stack: string }) =>
        setImmediate(() => {
          throw new Error(
            `Unexpected code block reached: ${err.message}\n` + err.stack
          );
        })
    );
  ...
  constructor(hostAddress: RabbitMqHostAddress, connectionName?: string) {
    ...
    this.busEndpoint = this.receiveEndpoint(
      `bus-${Guid.create().toString()}`,
      (config) => {
  ...
  async connect() {
    ...
    try {
      const a = connect(this.hostAddress + "?heartbeat=60", {
        clientProperties: { connection_name: this.connectionName },
      });
      this.connection = Promise.resolve(a);
...
const defaults: BusOptions = {
  host:
    process.env.RABBITMQ_HOST && process.env.RABBITMQ_HOST.length > 0
      ? process.env.RABBITMQ_HOST
      : "localhost",
  virtualHost:
    process.env.RABBITMQ_VHOST && process.env.RABBITMQ_VHOST.length > 0
      ? process.env.RABBITMQ_VHOST
      : "/",
};

function masstransit(options: BusOptions = defaults): Bus {
